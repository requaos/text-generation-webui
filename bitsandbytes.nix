...
buildPythonPackage {
  ...
  postPatch = ''
    substituteInPlace setup.py --replace 'version=f"0.38.0.post2",' "version='0.38.0',"
  ...
  preBuild = if torch.cudaSupport then
    ...
    ''
      echo "CUDA_VERSION=${cudaVersion} make cuda${cudaMajorVersion}x"
      CUDA_VERSION=${cudaVersion} make cuda${cudaMajorVersion}x
    ''
  else
